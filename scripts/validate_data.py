...
def validate_volumes(conn: sqlite3.Connection) -> bool:
    ...
    for table, min_count, max_count in checks:
        ...
        all_passed &= print_check(
            f"{table} count", passed, f"{count}, expected {min_count}-{max_count}"
        )
...
def validate_shift_risk(conn: sqlite3.Connection) -> bool:
    ...
    if day_rate > 0:
        ...
        all_passed &= print_check(
            "Night/Day incident ratio", passed, f"{night_ratio:.2f}x, expected ≥1.25x"
        )
    else:
        all_passed &= print_check(
            "Night/Day incident ratio", False, "No day shift data"
        )
...
def validate_employee_concentration(conn: sqlite3.Connection) -> bool:
    ...
    passed = top_4_pct >= 3
    all_passed = print_check(
        "Top 4 employees share of incidents", passed, f"{top_4_pct:.1f}%, expected ≥3%"
    )
...
def validate_data_quality(conn: sqlite3.Connection) -> bool:
    ...
    passed = bogus_emp >= 3
    all_passed &= print_check(
        "Bogus/missing employee refs exist", passed, f"Found {bogus_emp}"
    )
    ...
    all_passed &= print_check(
        "Bogus/missing machine refs exist", passed, f"Found {bogus_machine}"
    )
...
def validate_incident_types(conn: sqlite3.Connection) -> bool:
    ...
    machine_failure_variants = sum(
        1 for t in types if "fail" in t.lower() or "mech" in t.lower()
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Validate factory database characteristics"
    )
